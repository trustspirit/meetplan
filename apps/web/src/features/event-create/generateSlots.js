import { slotId, toZonedInstant } from "@meetplan/shared";
import { parseCellKey } from "./useEventCreateState";
export function buildSlotsFromPaintedCells(paintedCells, periodMinutes, tz) {
    const slots = [];
    for (const key of paintedCells) {
        const { dateYmd, hhmm } = parseCellKey(key);
        const startIso = toZonedInstant(dateYmd, hhmm, tz);
        const endIso = new Date(new Date(startIso).getTime() + periodMinutes * 60 * 1000).toISOString();
